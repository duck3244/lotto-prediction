...
def create_optimized_dataset(
    X: np.ndarray, y: np.ndarray, batch_size: int = 64, shuffle_buffer: int = 1000,
    seed: int = 42,
) -> tf.data.Dataset:
    """
    ...
    # 데이터셋 최적화 — seed 명시(TF_DETERMINISTIC_OPS=1 환경에서 필수)
    dataset = (dataset
              .cache()                                              # 메모리에 데이터셋 캐싱
              .shuffle(buffer_size=shuffle_buffer, seed=seed)       # 데이터 셔플 (결정성)
...
def get_latest_sequence(
    df: pd.DataFrame, sequence_length: int, scaler: MinMaxScaler,
) -> np.ndarray:
    """
